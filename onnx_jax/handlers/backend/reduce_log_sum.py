...
class ReduceLogSum(BackendHandler):
    @classmethod
    def _common(cls, node: OnnxNode, **kwargs):
        cls._rewrite(node)
        cls._prepare(node)

        return onnx_reduce_log_sum
    ...
    def _prepare(cls, node: OnnxNode):
        args = list(inspect.signature(onnx_reduce_log_sum).parameters.keys())
        attrs = [node.attrs.get(k, None) for k in args[node.len_inputs :]]
        node.attrs_list.extend(attrs)


@partial(jit, static_argnums=(1, 2))
def onnx_reduce_log_sum(data, axes=None, keepdims=1):
    return jnp.log(jnp.sum(data, axis=axes, keepdims=keepdims == 1))
