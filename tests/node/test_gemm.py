...
def gemm_reference_impl(A, B, C=None, alpha=1.0, beta=1.0, transA=0, transB=0):
...
class Gemm:
    ...
    def export_transposeA():  # type: () -> None
        ...
        b = np.random.ranf([6, 4]).astype(np.float32)
        c = np.zeros([1, 4]).astype(np.float32)
        y = gemm_reference_impl(a, b, c, transA=1)
    ...
    def export_alpha():  # type: () -> None
        ...
        b = np.random.ranf([5, 4]).astype(np.float32)
        c = np.zeros([1, 4]).astype(np.float32)
        y = gemm_reference_impl(a, b, c, alpha=0.5)
    ...
    def export_all_attributes():  # type: () -> None
        ...
        y = gemm_reference_impl(a, b, c, transA=1, transB=1, alpha=0.25, beta=0.35)
        expect(node, inputs=[a, b, c], outputs=[y], name='test_gemm_all_attributes')


if __name__ == '__main__':
    Gemm.export_all_attributes()
    Gemm.export_alpha()
    Gemm.export_beta()
    Gemm.export_default_matrix_bias()
    Gemm.export_default_no_bias()
    Gemm.export_default_scalar_bias()
    Gemm.export_default_single_elem_vector_bias()
    Gemm.export_default_vector_bias()
    Gemm.export_default_zero_bias()
    Gemm.export_transposeA()
    Gemm.export_transposeB()
